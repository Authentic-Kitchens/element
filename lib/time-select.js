...
var pickervue_type_script_lang_js_TIME_FORMATTER = function TIME_FORMATTER(value, format) {
  if (format === 'timestamp') return value.getTime();
  var date = new Date();
  ...
  return Object(date_util_["formatDate"])(date, format);
};
...
var TYPE_VALUE_RESOLVER_MAP = {
  ...
  'time-select': {
    formatter: pickervue_type_script_lang_js_TIME_FORMATTER,
    parser: TIME_PARSER
  },
  month: {
    formatter: pickervue_type_script_lang_js_DATE_FORMATTER,
    ...
    items: function items() {
      ...
      if (start && end && step) {
        if (compareTime(start, end) > 0) {
          ...
          var _current = start;
          while (compareTime(_current, end) <= 0) {
            result.push({
              value: _current,
              disabled: compareTime(_current, this.minTime || '-1:-1') <= 0 || compareTime(_current, this.maxTime || '100:100') >= 0
            });
            _current = nextTime(_current, step);
          }
        }
      }
