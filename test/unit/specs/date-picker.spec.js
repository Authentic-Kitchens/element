...
describe('DatePicker', () => {
  ...
  it('select date', (done) => {
    ...
    setTimeout((_) => {
      ...
      $el
        .querySelector('.el-date-picker__prev-btn.el-icon-d-arrow-left')
        .click()
      let count = 20
      while (--count) {
        arrowLeftElm.click()
  ...
  describe('type:datetime', () => {
    ...
    it('both picker show correct formated value (extract date-format and time-format from format property)', (done) => {
      vm = createVue(
        {
          template:
            '<el-date-picker type="datetime" v-model="value" :format="format" :pickerOptions="pickerOptions" ref="compo" />',
          data() {
            return {
    ...
    describe('cancel time', () => {
      ...
      it('cancel to empty', (done) => {
        ...
        setTimeout((_) => {
          const timeInput = vm.$refs.compo.picker.$el.querySelector(
            '.el-date-picker__time-header > span:nth-child(2) input'
          )
          timeInput.focus()
          setTimeout((_) => {
    ...
    it('select time honors picked date', (done) => {
      ...
      setTimeout((_) => {
        ...
        vm.$refs.compo.picker.$el
          .querySelector('.el-date-picker__header .el-icon-arrow-right')
          .click()
        setTimeout((_) => {
          vm.$refs.compo.picker.$el
            .querySelector('.el-date-picker__header .el-icon-d-arrow-right')
            .click()
          setTimeout((_) => {
  ...
  describe('type:datetimerange', () => {
    ...
    it('both picker show correct formated value (extract date-format and time-format from format property)', (done) => {
      ...
      setTimeout((_) => {
        ...
        compo.$el.click()
        setTimeout((_) => {
          const pickers = compo.picker.$el.querySelectorAll(
    ...
    it('prev/next month button', (done) => {
      const leftBtn = vm.picker.$el.querySelector(
        '.is-left .el-icon-arrow-left'
      )
      const rightBtn = vm.picker.$el.querySelector(
        '.is-right .el-icon-arrow-right'
      )
    ...
    it('prev/next year button', (done) => {
      const leftBtn = vm.picker.$el.querySelector(
        '.is-left .el-icon-d-arrow-left'
      )
      const rightBtn = vm.picker.$el.querySelector(
        '.is-right .el-icon-d-arrow-right'
      )
      const left = vm.picker.$el.querySelector(
        '.is-left .el-date-range-picker__header'
      )
    ...
    it('confirm honors disabledDate', (done) => {
      ...
      vm = createVue(
        {
          template:
            '<el-date-picker type="datetimerange" value-format="yyyy-MM-dd HH:mm:ss" v-model="value" :picker-options="pickerOptions" ref="compo" />',
          data() {
            return {
              pickerOptions: {
                disabledDate(date) {
  ...
  const testWeek = (i) =>
    it('picker-options:firstDayOfWeek ' + i, (done) => {
      vm = createTest(
        DatePicker,
        {
          pickerOptions: {
  ...
  describe('picker-options:className', () => {
    it('set custom class name', async () => {
      ...
      expect(
        vm.$refs.compo.picker.$el
          .querySelector('.el-date-table__row td')
          .className.indexOf('test-class') > -1
      ).to.be.true
    })
  })
})
